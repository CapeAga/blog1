...
export default function CategoryPostsPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    async function fetchCategoryAndPosts() {
      try {
        ...
        // 获取分类信息
        const categoriesResponse = await apiClient.blog.getCategories();
        const categoriesData = categoriesResponse as unknown as { items: Category[] };
        const foundCategory = categoriesData.items.find(cat => cat.slug === params.slug);
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/blog/categories/${params.slug}?page=${newPage}`);
    setPage(newPage);
    window.scrollTo(0, 0);
  };
  ...
  return (
    <div className="px-4 py-6 sm:px-6 md:container md:mx-auto md:py-8">
      ...
      <Breadcrumb className="mb-4">
        ...
        <BreadcrumbItem>
          <Link href="/blog/categories" className="text-muted-foreground hover:text-foreground hover:underline transition-colors">分类</Link>
        </BreadcrumbItem>
        <BreadcrumbItem>
          <span className="text-foreground font-medium">{category?.name || '加载中...'}</span>
        </BreadcrumbItem>
      </Breadcrumb>
          ...
          {totalPages > 1 && (
            <div className="mt-12 flex justify-center">
              <nav className="flex items-center gap-1 sm:gap-2">
                {page > 1 && (
                  <Link
                    href={`/blog/categories/${params.slug}?page=${page - 1}`}
